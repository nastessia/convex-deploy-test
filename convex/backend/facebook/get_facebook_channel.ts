...
export const getFacebookChannel = query({
  args: {
    id: v.optional(v.id("facebook_channels")),
...
export const getFacebookChannelHttp = httpAction(
  async (ctx, request) => {
    try {
      ...
      const result = await ctx.runQuery(internal.backend.facebook.get_facebook_channel.getFacebookChannel, {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: "Failed to fetch Facebook Channel",

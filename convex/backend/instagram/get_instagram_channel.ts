...
export const getInstagramChannel = query({
  args: {
    id: v.optional(v.id("instagram_channels")),
...
export const getInstagramChannelHttp = httpAction(
  async (ctx, request) => {
    try {
      ...
      const result = await ctx.runQuery(internal.backend.instagram.get_instagram_channel.getInstagramChannel, {
      ...
      if (!result) {
        return new Response(
          JSON.stringify({
            success: false,
            error: "Instagram Channel not found",
          }),
          {
            status: 404,
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: "Failed to fetch Instagram Channel",

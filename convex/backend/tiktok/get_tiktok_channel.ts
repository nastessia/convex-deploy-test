...
export const getTiktokChannel = query({
  args: {
    id: v.optional(v.id("tiktok_channels")),
...
export const getTiktokChannelHttp = httpAction(
  async (ctx, request) => {
    try {
      ...
      const result = await ctx.runQuery(internal.backend.tiktok.get_tiktok_channel.getTiktokChannel, {
      ...
      if (!result) {
        return new Response(
          JSON.stringify({
            success: false,
            error: "Tiktok Channel not found",
          }),
          {
            status: 404,
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: "Failed to fetch Tiktok Channel",

...
export const getXChannel = query({
  args: {
    id: v.optional(v.id("x_channels")),
...
export const getXChannelHttp = httpAction(
  async (ctx, request) => {
    try {
      ...
      const result = await ctx.runQuery(internal.backend.x.get_x_channel.getXChannel, {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: "Failed to fetch X Channel",

...
export const getYoutubeChannel = query({
  args: {
    id: v.optional(v.id("youtube_channels")),
...
export const getYoutubeChannelHttp = httpAction(
  async (ctx, request) => {
    try {
      ...
      const result = await ctx.runQuery(internal.backend.youtube.get_youtube_channel.getYoutubeChannel, {
